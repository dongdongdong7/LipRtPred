...
# # }
# # .searchMainChains(smi = "[C@@H]1([C@H](O)[C@H](OP(=O)(O)O)[C@@H](COP(O)(=O)OP(O)(=O)OCC(C)([C@@H](O)C(=O)NCCC(=O)NCCSC(=O)C/C=C\\CC(O)=O)C)O1)N1C=NC2C(N)=NC=NC1=2",
# #                   scriptPath = system.file("python", "molecule_operation.py", package = "LipRtPred"))
# .searchMainChains <- function(smi, scriptPath){
#   browser()
#   acyloxy_position <- .searchAcyloxy(smi = smi, scriptPath = scriptPath)
